...
def if_then_else(cond, then_, else_):
    ...
    assert(width == width_of(else_))
    assert(width_of(cond) == 1)
    then_xor_else = xor_gate(then_, else_)
    cond_wire = first_wire(cond)
    then_xor_else_if_cond_else_zero = make_block(width)
    gates.append([2*width, width] + wires_in(then_xor_else) + [cond_wire]*width + wires_in(then_xor_else_if_cond_else_zero) + ["MAND"])
    return xor_gate(then_xor_else_if_cond_else_zero, else_)

# by using variables, topsort order is enforced!
x = make_block(32)
replacement_dummy = make_block(32)
unused_input = make_block(64)
x_is_zero = all_zero(x)
is_dummy = xor_gate(block_of_single_wire(first_wire(x) + log_N), x_is_zero)
output = if_then_else(is_dummy, replacement_dummy, x)
unused_output = make_block(96)
